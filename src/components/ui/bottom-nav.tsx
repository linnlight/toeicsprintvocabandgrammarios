...
export function BottomNav() {
  ...
  const items = [
    { href: '/home' as const, icon: '⌂', label: copy.home },
    { href: '/review' as const, icon: '↻', label: copy.review },
    { href: '/progress' as const, icon: '▥', label: copy.progress },
  ];
  return (
    <View style={[styles.wrap, { paddingBottom: Math.max(insets.bottom, space.sm) }]}>
      <View style={styles.nav}>
        {items.map((item) => {
          ...
          return (
            <Pressable accessibilityRole="tab" accessibilityState={{ selected: active }} key={item.href} onPress={() => router.replace(item.href)} style={styles.item}>
